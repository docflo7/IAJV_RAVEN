...
Raven_Bot::Raven_Bot(Raven_Game* world, Vector2D pos, bool bNeural) :
	...
	m_bRecording(false), m_bTir(false)
{
	...
	m_pDatasetWriteRegulator = new Regulator(10); //save data twice per seconds
	...
	if (m_bNeural == true)
	{
		BPN::Network::Settings networkSettings{ 5, 5, 1 };
		m_nNetwork = new BPN::Network(networkSettings);

		BPN::TrainingDataReader dataReader("dataset.data", 5, 1);
		dataReader.ReadData();
...
void Raven_Bot::Update()
{
	...
	if (!isPossessed())
	{
		...
		if (m_bNeural)
		{
			...
			Raven_Bot* target = GetTargetSys()->GetTarget();
			if (target != nullptr) {
				Vector2D toTarget = Vec2DNormalize(target->Pos() - m_vPosition);
				double dot = m_vFacing.Dot(toTarget);
				Clamp(dot, -1, 1);
				double angle = acos(dot);
				double dist = Vec2DDistanceSq(target->Pos(), m_vPosition);
				bool visible = hasLOSto(target->Pos());

				debug_con << "Evalutate = " << m_nNetwork->Evaluate({ weaponType, weaponAmmo, dist, double(m_iHealth), double(visible) }).at(0) << "";
				if (m_nNetwork->Evaluate({ weaponType, weaponAmmo, dist, double(m_iHealth), double(visible) }) == std::vector<int32_t>{-1})
				{
...
Raven_Bot* Raven_Bot::GetClosestTargetToAim(POINT p) {
	Vector2D cursor = POINTtoVector(p);
	...
	for (curBot = m_pWorld->GetAllBots().begin(); curBot != m_pWorld->GetAllBots().end(); ++curBot) {
		if ((*this).ID() != (*curBot)->ID()) {
			double dist = Vec2DDistanceSq((*curBot)->Pos(), cursor);
...
void Raven_Bot::WriteDataSet(int tir) {
	...
	POINT p;
	GetCursorPos(&p);
	Raven_Bot* target = GetClosestTargetToAim(p);

	//calculate angle with target
	Vector2D toTarget = Vec2DNormalize(target->Pos() - m_vPosition);
	double dot = m_vFacing.Dot(toTarget);
	Clamp(dot, -1, 1);
	double angle = acos(dot);  //not used now
