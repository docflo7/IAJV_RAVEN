...
double Raven_Feature::TotalWeaponStrength(Raven_Bot* pBot)
{
  ...
  // NOTE: WTF? This was not even used before...
  ...
  const double TotalRounds = NumSlugs + NumCartridges + NumRockets + NumGrenades;
  ...
  return Tweaker + (1-Tweaker)*(TotalRounds)/(TotalRoundsCarryable);
}
