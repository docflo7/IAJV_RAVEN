...
void Raven_Game::AddPlayer()
{
	...
	//take possession of the new bot
	m_pSelectedBot = rb;
	m_pSelectedBot->ForcePossession();
...
//---------------------------StopAction----------------------------------------
// Stop every Mouse's action in querie
//-----------------------------------------------------------------------------
void Raven_Game::StopAction()
{
	m_pSelectedBot->GetBrain()->RemoveAllSubgoals();
}
...
//------------------------ ChangePlayerVelocity -------------------------------
//
//  if a bot is possessed any directionnal input change the velocity of the player
//-----------------------------------------------------------------------------
void Raven_Game::ChangePlayerVelocity()
{
	Vector2D newVelocity = Vector2D(0, 0);
	if (m_pSelectedBot && m_pSelectedBot->isPossessed())
	{
		if (UserOptions->m_bAzerty)
		{
			if (IS_KEY_PRESSED('Z')) {
				newVelocity = newVelocity + Vector2D(0, -1);
			}
			if (IS_KEY_PRESSED('S')) {
				newVelocity = newVelocity + Vector2D(0, 1);
			}
			if (IS_KEY_PRESSED('Q')) {
				newVelocity = newVelocity + Vector2D(-1, 0);
			}
			if (IS_KEY_PRESSED('D')) {
				newVelocity = newVelocity + Vector2D(1, 0);
			}
		...
		newVelocity = newVelocity*(m_pSelectedBot->MaxSpeed());
		m_pSelectedBot->SetVelocity(newVelocity);
	}
}
